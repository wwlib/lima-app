...
function SocketPanel({ model }: { model: Model }) {
    ...
    const onButtonClicked = (action: string, event: any) => {
        ...
        switch (action) {
            case 'Connect':
                setLimaServiceClient(model.getLimaSocketClientController(serviceUrl, authUrl, clientAccountId, clientPassword, true))
                break;
            case 'SendText':
                if (limaServiceClient) {
                    const commandData = {
    ...
    return (
        <div className="SocketPanel">
            <div className="SocketPanel-row">
                <textarea className="SocketPanel-messages" value={messages} readOnly rows={16} />
            </div>
            <div className="SocketPanel-row">
                <form className='form' onSubmit={(event: any) => { onButtonClicked('SendText', event) }}>
